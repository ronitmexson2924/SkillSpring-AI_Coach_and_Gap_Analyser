  ...
  async chatStream(payload, handlers = {}) {
    return streamRequest(
      ...
      async () => {
        ...
        const answer = `Start with ${priorities[0] ?? 'your highest gap'}, then reinforce it with one project and one reflection loop. Your next best move is to practice ${priorities[1] ?? 'communication'} in public by writing short build notes.`;
